    ...
    <div class="container">
        ...
        @if ($orders !== null)
                ...
                @foreach ($orders as $o)
                    <div class="order-card">
                        <div class="order-header">
                            <span class="order-id">#{{ $o->id }} • {{ \Carbon\Carbon::parse($o->created_at)->translatedFormat('d M Y, H:i') }}</span>
                            <span class="badge badge-{{ $o->status }}">
                                @switch($o->status)
                                    @case('pending') Menunggu @break
                                    @case('progress') Diproses @break
                        ...
                        <div class="order-detail">
                            <strong>{{ $o->theme_name ?? 'Undangan' }}</strong> — {{ $o->theme_category ?? '-' }}
                            <br />{{ $o->client_name ?? '-' }}
                        </div>
                        @if ($o->status === 'done' && $o->result_link)
                            <div style="margin-top:10px;">
                                <a href="{{ $o->result_link }}" target="_blank" style="display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#f59e0b,#d97706);color:#000;padding:8px 16px;border-radius:8px;text-decoration:none;font-weight:600;font-size:0.82rem;">
            ...
            @else
                ...
                Tidak ada pesanan ditemukan untuk nomor <strong>{{ $phone ?? '' }}</strong>.
                </div>
            @endif
        @else
            <div class="empty">
